...
#Full Species
microbiome_tree <- read.tree("sample-clustering-upgma_fullsp.tre") #this is an original tree with the root word in it #you can ignore the warming or follow this link here for the solution: https://github.com/YuLab-SMU/tidytree/issues/10
...
#RF      RFrd
#stat 0.5454545 0.5454545
#pval 0.0000000 0.0000000
...
asv <- read.delim("phylosym_table_group_fullsp.txt", sep = '\t', row.names = 1, header = T, strip.white = T)
asv_t <- t(asv)
...
#Method 1(my way):
bc_dist <- vegdist(asv_r[,1:3202], method = "bray")
