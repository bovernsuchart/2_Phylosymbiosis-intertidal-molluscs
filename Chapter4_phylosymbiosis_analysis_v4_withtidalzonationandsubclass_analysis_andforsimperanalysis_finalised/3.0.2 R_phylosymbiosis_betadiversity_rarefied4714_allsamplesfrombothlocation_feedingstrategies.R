...
levels(asv_rarefied4714_metadata$feeding_strategies)
...
adonis.microbiome <- adonis2(asv_rarefied4714_metadata[,1:6468] ~ asv_rarefied4714_metadata$feeding_strategies, method = "bray")
adonis.microbiome
...
pairwiseAdonis::pairwise.adonis(asv_rarefied4714_metadata[,1:6468], asv_rarefied4714_metadata$species_allknown, 
...
bdisp <- betadisper(rarefied4714_bray, asv_rarefied4714_metadata$feeding_strategies , type=c("centroid"))
...
#Groups     2 0.023405 0.0117025 9.8528    999  0.001 ***
#Residuals 71 0.084330 0.0011877 
...
pcoa.meio.bray.plot <- ggplot(bray.pcoa.eigen.plotting, aes(x = axis_1, y = axis_2, colour = feeding_strategies)) +
  ...
  xlab("PCoA 1 (8.91%)") +
  ylab("PCoA 2 (7.72)") +
  labs(color = "SFeeding Strategies") +
...
C3 <- c("#7AEA4A", "#B63DE7","#74B6DD")

pcoa.meio.bray.plot <- ggplot(gg, aes(x = axis_1.x, y = axis_2.x, colour = feeding_strategies, shape = location)) +
  geom_point(size = 3) +
  ...
  xlab("PCoA 1 (8.91%)") +
  ylab("PCoA 2 (7.72)") +
  labs(color = "Feeding stategies", shape = "Location") +
  scale_color_hue(labels = c("filter_feeder" = "Filter feeder",    #you can set his as scale_color_hue() but without the values argument which is needed for scale_fill_manual
                              "grazer" = "Grazers",
                             "predators" = "Predators")) +
   theme(axis.text.y = element_text(colour = "black", size = 12), 
        axis.text.x = element_text(colour = "black", size = 12), 
...
pcoa.meio.bray.plot + annotate("text", x = -0.3, y = -0.4, label = "PERMANOVA: p-value < 0.05\n PERMDISP: p-value < 0.05", size = 5)

ggsave(filename = "Rarefied_data_pcoa_plot_allsamples_feedingstrategies", width = 22, height = 16, units = "cm", device = "pdf", dpi = "print")
