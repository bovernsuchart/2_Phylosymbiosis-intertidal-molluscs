#Microbiome actual R thesis version - 01.11.2022 - tidal position from both locations
...
asv_rarefied4714_metadata$single_zone <-factor(asv_rarefied4714_metadata$single_zone, levels = c("High", "Mid", "Low"))
levels(asv_rarefied4714_metadata$single_zone)
...
adonis.microbiome <- adonis2(asv_rarefied4714_metadata[,1:6468] ~ asv_rarefied4714_metadata$single_zone, method = "bray")
adonis.microbiome

#                               Df SumOfSqs      R2     F Pr(>F)    
#asv_rarefied4714_metadata$single_zone  2    3.432 0.09975 3.9333  0.001 ***
#Residual                              71   30.979 0.90025                  
...
pairwiseAdonis::pairwise.adonis(asv_rarefied4714_metadata[,1:6468], asv_rarefied4714_metadata$single_zone, 
...
bdisp <- betadisper(rarefied4714_bray, asv_rarefied4714_metadata$single_zone , type=c("centroid"))
...
#Groups     2 0.037973 0.0189864 15.854    999  0.001 ***
#Residuals 71 0.085028 0.0011976 
...
pcoa.meio.bray.plot <- ggplot(gg, aes(x = axis_1.x, y = axis_2.x, colour = single_zone, shape = location)) +
  geom_point(size = 3) +
  ...
  xlab("PCoA 1 (8.91%)") +
  ylab("PCoA 2 (7.72)") +
  labs(color = "Tidal zone", shape = "Location") +
  scale_color_hue() +
   theme(axis.text.y = element_text(colour = "black", size = 12), 
        axis.text.x = element_text(colour = "black", size = 12), 
        legend.text = element_text(size = 12, colour ="black"), 
...
pcoa.meio.bray.plot + annotate("text", x = -0.4, y = 0.15, label = "PERMANOVA: p-value < 0.05\n PERMDISP: p-value < 0.05")

ggsave(filename = "Rarefied_data_pcoa_plot_allsamples_zones.pdf", width = 22, height = 16, units = "cm", device = "pdf", dpi = "print")
