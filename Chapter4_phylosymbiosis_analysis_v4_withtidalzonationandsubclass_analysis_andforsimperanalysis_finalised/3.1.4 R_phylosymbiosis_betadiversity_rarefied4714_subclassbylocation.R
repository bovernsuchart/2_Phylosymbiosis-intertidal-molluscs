#Microbiome actual R thesis version - 01.11.2022 - subclass from by locations (HK/TH)
...
adonis.microbiome <- adonis2(asv_rarefied4714_metadata_HK[,1:3864] ~ asv_rarefied4714_metadata_HK$subclass, method = "bray")
adonis.microbiome

#                                                Df SumOfSqs      R2      F Pr(>F)    
#asv_rarefied4714_metadata_HK$subclass  4    6.544 0.34854 4.9489  0.001 ***
#Residual                              37   12.231 0.65146                  
#Total                                 41   18.775 1.00000   
...
pairwiseAdonis::pairwise.adonis(asv_rarefied4714_metadata_HK[,1:3864], asv_rarefied4714_metadata_HK$subclass, 
...
bdisp <- betadisper(rarefied4714_bray_HK, asv_rarefied4714_metadata_HK$subclass, type=c("centroid"))
...
#Groups     4 0.50124 0.125311 23.695    999  0.001 ***
#Residuals 37 0.19567 0.005288                        
...
pcoa.meio.bray.plot_HK <- ggplot(bray.pcoa.eigen.plotting, aes(x = axis_1, y = axis_2, colour = subclass)) +
  geom_point(size = 3) +
...
pcoa.meio.bray.plot_HK <- ggplot(gg, aes(x = axis_1.x, y = axis_2.x, colour = subclass)) +
  geom_point(size = 3) +
...
ggsave(filename = "Rarefied4714_data_pcoa_plot_subclass_HKsamples", width = 18, height = 12, units = "cm", device = "pdf", dpi = "print")
...
#Note: 1. always check if the column is right! Make sure not to include empty count taxa 
#2. The result always changes because of the random permutation and so the significance return will always be different. 
adonis.microbiome <- adonis2(asv_rarefied4714_metadata_TH[,1:3105] ~ asv_rarefied4714_metadata_TH$subclass, method = "bray")
adonis.microbiome

#                                                Df SumOfSqs      R2      F Pr(>F)    
#asv_rarefied4714_metadata_TH$subclass  2   3.4632 0.24808 4.7839  0.001 ***
#Residual                              29  10.4968 0.75192                  
...
bdisp <- betadisper(rarefied4714_bray_TH, asv_rarefied4714_metadata_TH$subclass, type=c("centroid"))
bdisp   #This is with PCoA remember! and produce the interesting graph which seem significant and different from nmds above. 
aov.bdisp <-anova(bdisp)
aov.bdisp       
permutest(bdisp, pairwise = TRUE) #Not significant : How to interpret more: See this http://deneflab.github.io/MicrobeMiseq/demos/mothur_2_phyloseq.html
#Response: Distances
#           Df   Sum Sq   Mean Sq    F N.Perm Pr(>F)  
#Groups     2 0.27331 0.136656 26.065    999  0.001 ***
#Residuals 29 0.15204 0.005243    
...
pcoa.meio.bray.plot <- ggplot(gg, aes(x = axis_1.x, y = axis_2.x, colour = subclass)) +
  geom_point(size = 3) +
  geom_point(aes(x=axis_1.y,y=axis_2.y),size=2)+
  geom_segment(aes(x=axis_1.y, y=axis_2.y, xend=axis_1.x, yend=axis_2.x)) +
  theme_bw() + 
...
ggsave(filename = "Rarefied4714_data_pcoa_plot_subclass_THsamples", width = 18, height = 12, units = "cm", device = "pdf", dpi = "print")
