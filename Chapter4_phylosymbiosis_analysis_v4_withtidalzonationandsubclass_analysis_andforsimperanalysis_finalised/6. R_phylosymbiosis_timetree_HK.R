...
#HK Species
microbiome_tree <- read.tree("sample-clustering-upgma_HK.tre") #this is an original tree with the root word in it #you can ignore the warming or follow this link here for the solution: https://github.com/YuLab-SMU/tidytree/issues/10
...
#RF      RFrd
#stat 0.7142857 0.7142857
#pval 0.0140000 0.0341000
...
asv <- read.delim("phylosym_table_group_HK.txt", sep = '\t', row.names = 1, header = T, strip.white = T)
asv_t <- t(asv)
...
#Method 1(my way):
bc_dist <- vegdist(asv_r[,1:2121], method = "bray")
