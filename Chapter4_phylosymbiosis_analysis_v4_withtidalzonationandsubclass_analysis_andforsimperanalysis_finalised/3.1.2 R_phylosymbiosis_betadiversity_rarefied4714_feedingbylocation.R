...
adonis.microbiome <- adonis2(asv_rarefied4714_metadata_HK[,1:3864] ~ asv_rarefied4714_metadata_HK$feeding_strategies, method = "bray")
adonis.microbiome

#                                                Df SumOfSqs      R2      F Pr(>F)    
#asv_rarefied4714_metadata_HK$feeding_strategies  2   3.0706 0.16355 3.8127  0.001 ***
#Residual                                        39  15.7047 0.83645                  
...
pairwiseAdonis::pairwise.adonis(asv_rarefied4714_metadata_HK[,1:3864], asv_rarefied4714_metadata_HK$feeding_strategies, 
...
bdisp <- betadisper(rarefied4714_bray_HK, asv_rarefied4714_metadata_HK$feeding_strategies, type=c("centroid"))
...
#Groups     2 0.11742 0.058711 16.568    999  0.001 ***
#Residuals 39 0.13820 0.003544                          
...
pcoa.meio.bray.plot_HK <- ggplot(bray.pcoa.eigen.plotting, aes(x = axis_1, y = axis_2, colour = feeding_strategies)) +
  geom_point(size = 3) +
  #stat_ellipse(level = 0.95) +
  theme_bw() + 
  xlab("PCoA 1 (13.9793%)") +
  ylab("PCoA 2 (10.57868%)") +
  labs(color = "Feeding strategies") +
...
centroids <- aggregate(cbind(axis_1,axis_2)~feeding_strategies ,bray.pcoa.eigen.plotting, mean)

gg <- merge(bray.pcoa.eigen.plotting, centroids, by = "feeding_strategies" )
...
pcoa.meio.bray.plot_HK <- ggplot(gg, aes(x = axis_1.x, y = axis_2.x, colour = feeding_strategies)) +
  geom_point(size = 3) +
  ...
  scale_color_discrete(labels = c("filter_feeder" = "Filter feeder",
                                  "grazer" = "Grazer",
                                  "predators" = "Predators"
                                  )) +
  theme(axis.text.y = element_text(colour = "black", size = 12), 
...
ggsave(filename = "Rarefied4714_data_pcoa_plot_Feeding_HKsamples", width = 18, height = 12, units = "cm", device = "pdf", dpi = "print")
...
# re-order columns by total - check the last column
asv_sort <- asv_w_total[,order(-asv_w_total[which(rownames(asv_w_total) == 'total'), ])] #Here essentially it is re-ordering the total by descending order
asv_sort <- asv_sort[-33,] #remove the "total" row
...
#Note: 1. always check if the column is right! Make sure not to include empty count taxa 
#2. The result always changes because of the random permutation and so the significance return will always be different. 
adonis.microbiome <- adonis2(asv_rarefied4714_metadata_TH[,1:3105] ~ asv_rarefied4714_metadata_TH$feeding_strategies, method = "bray")
adonis.microbiome

#                                                Df SumOfSqs      R2      F Pr(>F)    
#asv_rarefied4714_metadata_TH$feeding_strategies  2   2.6336 0.18865 3.3716  0.001 ***
#Residual                                        29  11.3263 0.81135                  
#Total                                           31  13.9599 1.00000  

#Post-hoc test: I plan to use fdr as correction method because bonferroni can be a bit strict.
#But I proceed with fdr which was used in qiime2 
pairwiseAdonis::pairwise.adonis(asv_rarefied4714_metadata_TH[,1:3105], asv_rarefied4714_metadata_TH$species, 
                                p.adjust.m ='fdr', sim.method = 'bray') 
#The comparison is huge - but many with significance

#2. PERMDISP######################################################################################################################################################################################################################################################################################
rarefied4714_bray_TH <- vegdist(asv_rarefied4714_metadata_TH[,1:3105], method = "bray") 
bdisp <- betadisper(rarefied4714_bray_TH, asv_rarefied4714_metadata_TH$feeding_strategies, type=c("centroid"))
bdisp   #This is with PCoA remember! and produce the interesting graph which seem significant and different from nmds above. 
aov.bdisp <-anova(bdisp)
aov.bdisp       
permutest(bdisp) #Not significant : How to interpret more: See this http://deneflab.github.io/MicrobeMiseq/demos/mothur_2_phyloseq.html
#Response: Distances
#           Df   Sum Sq   Mean Sq    F N.Perm Pr(>F)  
#Groups     2 0.018422 0.0092110 3.8309    999  0.023 *
#Residuals 29 0.069727 0.0024044
...
# calculate the proportion of variance in the data which is explained by the first two PCoA axes
#Understand what is eigenvalue and eigenvector in here: https://www.youtube.com/watch?v=FgakZw6K1QQ
(bray.pcoa.eigen$eig[1]/(sum(bray.pcoa.eigen$eig)))*100 #16.10601%

(bray.pcoa.eigen$eig[2]/(sum(bray.pcoa.eigen$eig)))*100 #13.6479%

#Version 1: create a PCoA plot - no spiders
pcoa.meio.bray.plot <- ggplot(bray.pcoa.eigen.plotting, aes(x = axis_1, y = axis_2, colour = feeding_strategies)) +
  geom_point(size = 3) +
  #stat_ellipse(level = 0.95) +
  theme_bw() + 
  xlab("PCoA 1 (16.11%)") +
...
pcoa.meio.bray.plot <- ggplot(gg, aes(x = axis_1.x, y = axis_2.x, colour = feeding_strategies)) +
  geom_point(size = 3) +
  geom_point(aes(x=axis_1.y,y=axis_2.y),size=2)+
  geom_segment(aes(x=axis_1.y, y=axis_2.y, xend=axis_1.x, yend=axis_2.x)) +
  theme_bw() + 
...
ggsave(filename = "Rarefied4714_data_pcoa_plot_Feeding_THsamples", width = 18, height = 12, units = "cm", device = "pdf", dpi = "print")
