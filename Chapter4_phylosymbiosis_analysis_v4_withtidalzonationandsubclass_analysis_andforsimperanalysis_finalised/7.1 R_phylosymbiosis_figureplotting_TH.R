...
a <- data.frame(label = tiplabels_host,
                label2 = c( "I.", "Saccostrea", "M.", "C.", "T.", "E.", "E." ),
                label3 = c( "nucleus", "sp.","virgata","toreuma","musiva", "radiata", "malaccana"))
a
...
host_tree_plot <- ggtree(host_tree,branch.length = "none") %<+%  a + #the values is adjustable and depends on the how combine figure would like - so you may need to adjust accordingly 
  geom_tiplab(offset = 0.15, aes(label= paste0('italic(', label2, ')~italic(', label3, ')')), parse = TRUE) + 
  geom_nodepoint() + 
  geom_tippoint(aes(color = label3), size=3, show.legend = FALSE) + 
  #geom_nodelab(aes(label = label), nudge_x = 0.2) +
  geom_treescale(x=0.75, y=1, offset = 0.25) + 
  xlim(0, 5.5) + #becareful of this (you would want to test few time by changing the values so that all the information are covered)
  theme_tree() 
host_tree_plot

## Microbiome dendrogram - BrayCurtis-upgma
a_bc <- read.tree("sample-clustering-upgma_TH.tre") #this is an original tree with the root word in it #you can ignore the warming or follow this link here for the solution: https://github.com/YuLab-SMU/tidytree/issues/10
...
a_bc_rotated <- rotate(a_bc,13)
plot.phylo(a_bc_rotated) 
nodelabels()
a_bc_rotated_2 <- rotate(a_bc_rotated,10)
...
tiplabels_microbiome
a_b <- data.frame(label = tiplabels_microbiome, 
                  label2 = c( "M.", "Saccostrea", "I.", "C.","T.", "E.", "E."),
                  label3 = c(  "virgata", "sp.", "nucleus","toreuma", "musiva", "malaccana", "radiata"))
a_b
...
den_a_bc <- ggtree(a_bc,branch.length = "none") %<+% a_b + 
  ...
  scale_x_reverse(limits=c(7.6,0)) + 
  theme_tree() 
den_a_bc
...
asc1 <- ggdraw() + 
  ...
  draw_label("nRF = 0.0, p < 0.05 \n\nMantel r = 0.82, p < 0.05" , size = 11, x = 0.5, y = 0.3)
asc1

ggsave(filename = "Phylosymbiosis_timetreehosttree_TH.pdf", device = "pdf", width = 24, height = 15, units = "cm", dpi = "print")
