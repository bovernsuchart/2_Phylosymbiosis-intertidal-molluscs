#Microbiome actual R thesis version - 01.11.2022 - tidal position from by locations (HK/TH)
...
adonis.microbiome <- adonis2(rarefied4714_bray_HK ~ asv_rarefied4714_metadata_HK$single_zone)
adonis.microbiome

#                                                Df SumOfSqs      R2      F Pr(>F)    
#asv_rarefied4714_metadata_HK$single_zone  2   2.5144 0.13392 3.0153  0.001 ***
#Residual                                 39  16.2609 0.86608                  
...
pairwiseAdonis::pairwise.adonis(asv_rarefied4714_metadata_HK[,1:3864], asv_rarefied4714_metadata_HK$single_zone, 
...
bdisp <- betadisper(rarefied4714_bray_HK, asv_rarefied4714_metadata_HK$single_zone, type=c("centroid"))
...
#Groups     2 0.160332 0.080166 106.51    999  0.001 ***
#Residuals 39 0.029355 0.000753                         
...
pcoa.meio.bray.plot_HK <- ggplot(bray.pcoa.eigen.plotting, aes(x = axis_1, y = axis_2, colour = single_zone)) +
  geom_point(size = 3) +
...
centroids <- aggregate(cbind(axis_1,axis_2)~single_zone ,bray.pcoa.eigen.plotting, mean)

gg <- merge(bray.pcoa.eigen.plotting, centroids, by = "single_zone" )
...
pcoa.meio.bray.plot_HK <- ggplot(gg, aes(x = axis_1.x, y = axis_2.x, colour = single_zone)) +
  geom_point(size = 3) +
...
#ggsave(filename = "Rarefied4714_data_pcoa_plot_zone_HKsamples", width = 18, height = 12, units = "cm", device = "pdf", dpi = "print")
...
#Note: 1. always check if the column is right! Make sure not to include empty count taxa 
#2. The result always changes because of the random permutation and so the significance return will always be different. 
adonis.microbiome <- adonis2(asv_rarefied4714_metadata_TH[,1:3105] ~ asv_rarefied4714_metadata_TH$single_zone, method = "bray")
adonis.microbiome

#                                                Df SumOfSqs      R2      F Pr(>F)    
#asv_rarefied4714_metadata_TH$single_zone  2   3.6051 0.25825 5.0483  0.001 ***
#Residual                                 29  10.3548 0.74175                  
...
bdisp <- betadisper(rarefied4714_bray_TH, asv_rarefied4714_metadata_TH$single_zone, type=c("centroid"))
bdisp   #This is with PCoA remember! and produce the interesting graph which seem significant and different from nmds above. 
aov.bdisp <-anova(bdisp)
aov.bdisp       
permutest(bdisp, pairwise = TRUE) #Not significant : How to interpret more: See this http://deneflab.github.io/MicrobeMiseq/demos/mothur_2_phyloseq.html
#Response: Distances
#           Df   Sum Sq   Mean Sq    F N.Perm Pr(>F)  
#Groups      2 0.81085 0.40542 141.81    999  0.001 ***
#Residuals 29 0.08291 0.00286    
...
#Version 1: create a PCoA plot - no spiders
pcoa.meio.bray.plot <- ggplot(bray.pcoa.eigen.plotting, aes(x = axis_1, y = axis_2, colour = single_zone)) +
  geom_point(size = 3) +
  #stat_ellipse(level = 0.95) +
  theme_bw() + 
  xlab("PCoA 1 (16.11%)") +
...
pcoa.meio.bray.plot <- ggplot(gg, aes(x = axis_1.x, y = axis_2.x, colour = single_zone)) +
  geom_point(size = 3) +
  geom_point(aes(x=axis_1.y,y=axis_2.y),size=2)+
  geom_segment(aes(x=axis_1.y, y=axis_2.y, xend=axis_1.x, yend=axis_2.x)) +
  theme_bw() + 
...
ggsave(filename = "Rarefied4714_data_pcoa_plot_Zone_THsamples", width = 18, height = 12, units = "cm", device = "pdf", dpi = "print")
