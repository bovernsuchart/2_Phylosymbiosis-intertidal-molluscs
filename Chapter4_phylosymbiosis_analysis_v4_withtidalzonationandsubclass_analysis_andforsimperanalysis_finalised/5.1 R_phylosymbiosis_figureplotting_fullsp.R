...
a <- data.frame(label = tiplabels_host,
                label2 = c("I.","S.", "Saccostrea","M.","B.", "C.", "S.","T.", "R.", "E.", "E.","M.", "L." ),
                label3 = c( "nucleus","cucullata","sp.","virgata","virescens","toreuma","laciniosa", "musiva", "clavigera", "radiata", "malaccana", "labio", "granulata"))
a
...
host_tree_plot <- ggtree(host_tree,branch.length = "none") %<+%  a + #the values is adjustable and depends on the how combine figure would like - so you may need to adjust accordingly 
  geom_tiplab(offset = 0.25, aes(label= paste0('italic(', label2, ')~italic(', label3, ')')), parse = TRUE) + 
  geom_nodepoint() + 
  geom_tippoint(aes(color = label3), size=3, show.legend = FALSE) + 
  #geom_nodelab(aes(label = label), nudge_x = 0.2) +
  geom_treescale(x=0.5, y=1, offset = 0.25) + 
  xlim(0, 8.1) + #becareful of this (you would want to test few time by changing the values so that all the information are covered)
  theme_tree() 
host_tree_plot

## Microbiome dendrogram - BrayCurtis-upgma
a_bc <- read.tree("sample-clustering-upgma_fullsp.tre") #this is an original tree with the root word in it #you can ignore the warming or follow this link here for the solution: https://github.com/YuLab-SMU/tidytree/issues/10
...
a_bc_rotated <- rotate(a_bc,23)
plot.phylo(a_bc_rotated) 
nodelabels()
a_bc_rotated_2 <- rotate(a_bc_rotated,24)
plot.phylo(a_bc_rotated_2) 
nodelabels()
a_bc_rotated_3 <- rotate(a_bc_rotated_2,17)
plot.phylo(a_bc_rotated_3) 
nodelabels()
a_bc_rotated_4 <- rotate(a_bc_rotated_3,15)
plot.phylo(a_bc_rotated_4) 
nodelabels() #perfect
...
tiplabels_microbiome
a_b <- data.frame(label = tiplabels_microbiome, 
                  label2 = c("L.", "M.", "M.", "B.", "I.", "S.", "Saccostrea","C.", "T.", "E.","E.","S.", "R."),
                  label3 = c( "granulata", "labio", "virgata","virescens","nucleus","cucullata","sp.","toreuma","musiva", "malaccana", "radiata", "laciniosa", "clavigera" ))
a_b

den_a_bc <- ggtree(a_bc,branch.length = "none") %<+% a_b + 
  geom_tiplab(offset = -3.9, aes(label= paste0('italic(', label2, ')~italic(', label3, ')')), parse = TRUE) + #the values is adjustable and depends on the how combine figure would like - so you may need to adjust accordingly. Parse = T: is important for the paste0 to work
  geom_nodepoint() + 
...
asc1 <- ggdraw() + 
  ...
  draw_plot(den_a_bc, x = 0.5, y = 0.35, width = 0.45, height = 0.6) + draw_label("Microbial dendrogram", size=13, x=0.62, y=0.975) +
  draw_label("nRF = 0.55, p < 0.05 \n\nMantel r = 0.58, p < 0.05", size = 11, x = 0.5, y = 0.3)
asc1  

ggsave(filename = "Phylosymbiosis_timetreehosttree_fullsp.pdf", device = "pdf", width = 24, height = 15, units = "cm", dpi = "print")
